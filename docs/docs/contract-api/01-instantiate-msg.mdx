...
end on the counterparty chain, then the owner must call [`ExecuteMsg::CreateChannel`](./02-execute-msg.mdx#create-channel) with the correct parameters to start a new channel open handshake.
...
Learn more about callbacks [here](/contract-api/callbacks).
